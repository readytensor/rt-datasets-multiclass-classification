...
def gen_cl_multi_base_schemas(schema_cfg, root_path):
    ...
    for dataset in datasets:
        ...
        output_fpath = os.path.join(
            root_path, "datasets", dataset, "processed", f"{dataset}_schema.json"
        )

        input_dict = schema_dict["inputDatasets"][
            "multiClassClassificationBaseMainInput"
        ]
        ...
        for _, row in schema_params.iterrows():
            ...
            if row[use_type_lbl] == field_use_types["id"]:
                ...
                if target_attr is not None:
                    raise Exception(
                        f"Error: Only 1 target attr is permissible. Can't set field '{row[field_name_lbl]}' as {row[use_type_lbl]}"
                    )
...
if __name__ == "__main__":
    ...
    gen_cl_multi_base_schemas(schema_cfg, root_path)

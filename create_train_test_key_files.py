...
def split_dataset(
    ...
    """
    Splits the dataset into train and test set, ensuring that all classes are represented in both sets.

    Args:
        dataset (DataFrame): The dataset to be split.
        target_name (str): The name of the column in the DataFrame that holds the class labels.
...
def create_train_test_testkey_files(dataset_cfg_path:str, processed_datasets_path:str) -> None:
    ...
    for _, dataset_row in dataset_metadata[dataset_metadata['use_dataset'] == 1].iterrows():
        ...
        # Split the dataset
        train_df, test_df = split_dataset(dataset, dataset_row["target_name"])
